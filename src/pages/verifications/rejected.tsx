...
import { useGetAllRejectedVerificationsQuery } from '../../generated/graphql'
...
const RejectedVerificationsIndexPage: NextPage = () => {
  const [after, setAfter] = useState<string>('')
  const [before, setBefore] = useState<string>('')
  const [searchText, setSearchText] = useState<string>('')
  const [{ data }] = useGetAllRejectedVerificationsQuery({
    variables: { options: { first: 10, after, before, query: searchText } },
  })

  const gotoPrevPage = useCallback(() => {
    setBefore(data?.getAllRejectedVerifications?.pageInfo?.startCursor || '')
    setAfter('')
  }, [data?.getAllRejectedVerifications?.pageInfo?.startCursor])

  const gotoNextPage = useCallback(() => {
    setAfter(data?.getAllRejectedVerifications?.pageInfo?.endCursor || '')
    setBefore('')
  }, [data?.getAllRejectedVerifications?.pageInfo?.endCursor])

  const userData =
    data?.getAllRejectedVerifications?.edges?.map((edge) => ({
  ...
  return (
    <AdminLayout>
      ...
      <DataTable
        ...
        columns={columns}
        data={userData}
        hasNextPage={data?.getAllRejectedVerifications?.pageInfo?.hasNextPage || false}
        hasPreviousPage={data?.getAllRejectedVerifications?.pageInfo?.hasPreviousPage || false}
...
export default withUrqlClient(createUrqlClient)(RejectedVerificationsIndexPage)
