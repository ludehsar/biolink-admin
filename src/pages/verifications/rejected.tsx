...
const RejectedVerificationsIndexPage: NextPage = () => {
  ...
  const gotoPrevPage = useCallback(() => {
    setBefore(data?.getAllRejectedVerifications?.cursor.beforeCursor || '')
    setAfter('')
  }, [data?.getAllRejectedVerifications?.cursor.beforeCursor])

  const gotoNextPage = useCallback(() => {
    setAfter(data?.getAllRejectedVerifications?.cursor.afterCursor || '')
    setBefore('')
  }, [data?.getAllRejectedVerifications?.cursor.afterCursor])

  const userData =
    data?.getAllRejectedVerifications?.data.map((verification) => ({
      id: verification.id,
  ...
  return (
    <AdminLayout>
      ...
      <DataTable
        ...
        columns={columns}
        data={userData}
        hasNextPage={!!data?.getAllRejectedVerifications?.cursor.afterCursor}
        hasPreviousPage={!!data?.getAllRejectedVerifications?.cursor.beforeCursor}
