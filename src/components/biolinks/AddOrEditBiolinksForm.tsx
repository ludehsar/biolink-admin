...
const AddOrEditBiolinksForm: React.FC<AddOrEditBiolinksFormProps> = ({
  ...
  const [{ data: userData }] = useGetUserQuery({ variables: { userId: userId as string } })
          ...
          initialValues={{
            userId:
              variant === 'Add'
                ? userId
                  ? userId
                  : ''
                : (biolinkData?.getBiolink?.user?.id as string),
            username:
              variant === 'Add' ? '' : (biolinkData?.getBiolink?.username?.username as string),
            displayName: variant === 'Add' ? '' : (biolinkData?.getBiolink?.displayName as string),
            categoryId: variant === 'Add' ? '' : (biolinkData?.getBiolink?.category?.id as string),
            ...
            addedToDirectory:
              variant === 'Add'
                ...
                : 'false',
            directoryBio:
              variant === 'Add' ? '' : (biolinkData?.getBiolink?.settings?.directoryBio as string),
            ...
            payoneerLink:
              variant === 'Add' ? '' : (biolinkData?.getBiolink?.settings?.payoneerLink as string),
            paypalLink:
          ...
          onSubmit={async (values, { setSubmitting }) => {
            if (variant === 'Add') {
              ...
              if (response.error) {
                ...
                if (!userId) return router.push('/biolinks')
              }
            } else {
                ...
                <CardBody>
                  ...
                  <div className="pl-lg-4">
                    <Row>
                      <Col lg="6">
                        <FormGroup>
                          <label className="form-control-label" htmlFor="blockSearchEngineIndexing">
                  ...
                  <div className="pl-lg-4">
                    <Row>
                      <Col lg="6">
                        <FormGroup>
                          <label className="form-control-label" htmlFor="enablePasswordProtection">
